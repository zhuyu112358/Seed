// Seed Test World Demo - uses WorldEngine directly.
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < boxes.length; i++) {
    engine.createEntity({ type: "dynamic", name: `box-${i}`, position: boxes[i], mass: 1, material: "wood" });
  }
  ...
  for (let i = 0; i < TICKS; i++) {
    ...
    if (i % 30 === 0) {
      ...
      const soul = engine.getEntity("soul:local-test");
      const pos = soul ? `(${soul.position.x.toFixed(1)}, ${soul.position.y.toFixed(1)}, ${soul.position.z.toFixed(1)})` : "(none)";
      console.log(`[${ts()}] tick=${i} entities=${stats.entityCount} soul=${pos}`);
    }
  }
  const stats = engine.getStats();
