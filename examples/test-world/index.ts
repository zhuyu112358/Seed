...
async function main(): Promise<void> {
  ...
  // Pull soul roster (fall back to mocks if SoulArena is offline).
  ...
  for (const [id, e] of world.entities) {
    ...
    console.log(`${e.name.padEnd(12)} -> (${e.position.x.toFixed(2)}, ${e.position.y.toFixed(2)}, ${e.position.z.toFixed(2)})  螖=(${dx.toFixed(2)}, ${dy.toFixed(2)})`);
  }
  console.log(`\ncollisions: ${collisionCount}, zone events: ${zoneEvents}`);

  // Acoustic communication demo: Vex speaks, Nova hears depending on distance.
  ...
  const received = acoustic.transmit(
    msg,
    vexProxy as never,
    { entities: world.bodies(), byId: (id) => world.getEntity(id) as never },
  );
  console.log('\n--- Acoustic demo ---');
  console.log(`Vex says: "${msg.content}" from ${vexProxy.position.toString()}`);
  for (const r of received) {
    console.log(`  -> received at distance ${r.distance.toFixed(2)}m, intensity ${r.receivedIntensity.toFixed(3)}`);
  }
