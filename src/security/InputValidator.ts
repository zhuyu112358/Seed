// InputValidator: tiny schema-based validator used for soul-action payloads and
// REST bodies. Aims to be dependency-free and strict about types/ranges/lengths.
...
export class InputValidator {
  validate(schema: Schema, input: unknown): ValidationResult {
    ...
    if (typeof input !== 'object' || input === null || Array.isArray(input)) {
      return { ok: false, errors: ['input must be an object'], value: {} };
    }
    ...
    for (const [field, rule] of Object.entries(schema)) {
      ...
      if (rule.type === 'string' && typeof raw === 'string') {
        if (rule.min !== undefined && raw.length < rule.min) errors.push(`"${field}" too short`);
        if (rule.max !== undefined && raw.length > rule.max) errors.push(`"${field}" too long`);
        if (rule.pattern && !rule.pattern.test(raw)) errors.push(`"${field}" failed pattern`);
      }
      if (rule.type === 'number' && typeof raw === 'number') {
        if (rule.min !== undefined && raw < rule.min) errors.push(`"${field}" below min`);
        if (rule.max !== undefined && raw > rule.max) errors.push(`"${field}" above max`);
      }
      if (rule.enum !== undefined && !rule.enum.includes(raw as string | number | boolean)) {
        errors.push(`"${field}" not in allowed set`);
      }
      value[field] = raw;
    }
