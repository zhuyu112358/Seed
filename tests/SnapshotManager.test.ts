...
describe('SnapshotManager', () => {
  ...
  it('save writes a JSON file and load reads it back', () => {
    ...
    assert.equal(snap.schema, 'seed/world-snapshot@1');
  });
  ...
  it('list returns snapshots newest first', async () => {
    ...
    // Newest first: load the first file and expect the highest tick.
    const newest = mgr.load(files[0]);
    assert.equal(newest.tick, 3);
  });
  ...
  it('rollback loads the most recent snapshot', async () => {
    ...
    const snap = mgr.rollback();
    assert.ok(snap);
    assert.equal(snap!.tick, 20);
  });
  ...
  it('keep limits retained snapshots', async () => {
    ...
    const files = mgr.list().filter((f) => f.startsWith('k-'));
