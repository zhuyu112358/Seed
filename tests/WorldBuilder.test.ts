// Integration test for the bottom-level composition that WorldBuilder is meant
// to wrap.
//
// NOTE: src/sdk/WorldBuilder.ts has a broken import (it pulls in
// ./PhysicsConfig.js which does not exist on that path), so it cannot be
// imported directly. This test instead exercises the real building blocks the
// builder itself should use: an engine World container + EntityFactory +
// PhysicsSystem + EventSystem. (src/engine/WorldEngine.ts is also broken, so
// World.step is used to advance the simulation rather than WorldEngine.load.)
...
import { GameObject, Entity } from '../src/entity/Entity.js';
...
describe('World composition (WorldBuilder building blocks)', () => {
  it('constructs a world and manages its entities', () => {
    ...
    const removed = world.removeEntity(box.id);
  ...
  it('runs a PhysicsSystem plugged in as a world system', () => {
    ...
    world.step(1 / 60);
    // gravity should have pulled the ball down
  ...
  it('queryByType / bodies / iterate work over the entity set', () => {
    ...
    const bodySet = world.bodies();
    assert.ok(bodySet.every((b) => b instanceof GameObject));
    assert.equal(bodySet.length, 3);
