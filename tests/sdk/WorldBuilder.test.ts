...
test(''addEntity stores pending entities and returns ids'', () => {
  ...
  assert.ok(id1); assert.ok(id2); assert.notEqual(id1, id2);
  const ids = builder.addEntities([{ type: ''dynamic'', name: ''c'' }, { type: ''dynamic'', name: ''d'' }]);
  assert.equal(ids.length, 2);
});

test(''setPhysicsConfig merges overrides over defaults'', () => {
  const config = new WorldBuilder().createWorld({ name: ''p'' })
    .setPhysicsConfig({ gravity: { x: 0, y: -1.62, z: 0 } }).build();
...
test(''registerSoul queues a soul anchor for startup'', async () => {
  const builder = new WorldBuilder().createWorld({ name: ''soul'' }).registerSoul(''alpha'', { x: 1, y: 2, z: 3 });
